...
// 页面模版
const indexTep = `import Taro,{ useState, useEffect } from '@tarojs/taro';
...
export default connect(({ ${dirName}, loading })=>({ ${dirName},loading }))(${titleCase(dirName)})`;
// less文件模版
...
export default {
  ...
  effects: {
    * effectsDemo(_, { call, put }) {
      const { success, data } = yield call(${dirName}Api.demo, {});
      if (success ) {
        yield put({ type: 'save',
          payload: {
            keai: data,
